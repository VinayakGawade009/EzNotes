...
function AIChatBox({ open, onClose }: AIChatBoxProps) {
  ...
  }, [open, messages]); // we want to scroll down only when open changes or messages arrat changes
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !e.shiftKey) { // Enter -> sends message, shift + Enter -> new line
      onSubmit(e);
    }
  };
  ...
  return (
    ...
    >
      <div className="bg-primary text-primary-foreground flex items-center justify-between rounded-t-lg border-b p-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => setMessages(initialMessages)} // after deleting previous chat history we still want initial welcome message in aiChatBox
            className="text-primary-foreground hover:bg-primary/90 h-8 w-8"
            title="Clear chat"
            disabled={isProcessing}
          >
            <Trash />  {/* deletes messages from aiChatBox */}
          </Button>
          <Button
            variant="ghost"
            size="icon"
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        ...
        { status === "submitted" && lastMessageIsUser && <Loader /> } {/* loading indicator or ai message */}
      ...
      <form className="flex gap-2 border-t p-3" onSubmit={onSubmit}>
        ...
        <Button 
          type="submit" 
          size="icon"
          disabled={!input.trim() || isProcessing} // disables when no message to send or ai is processing
        >
